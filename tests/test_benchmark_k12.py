...
message = Message(
    message_id=30,
    text="a man riding a bicycle near the beach on a sunny day",
    tokens=(
        "a", "man", "riding", "a",
        "bicycle", "near", "the", "beach",
        "on", "a", "sunny", "day",
    ),
)
...
def evaluate(label, result, encoder):
    ...
    print(f"Batches:   {result.encoder_batches}")
...
print("\n")
print("=" * 80)
print("K=12 / M=4 SEMPA-LAB BASELINE")
print("=" * 80)
...
assert result_k0.logical_calls == 20
assert result_k1.logical_calls == 40
assert result_k2.logical_calls == 50

print("\n✅ K=12 benchmark passed")